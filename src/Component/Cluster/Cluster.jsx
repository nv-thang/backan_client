...
export default function Cluster() {
	...
	const fetchData = async () => {
		const url = "http://localhost:4004/cluster/getAllCluster"
		try {
	...
	const handleEditClick = async (rowData) => {
		...
		const url = `http://localhost:4004/cluster/getClusterById?id=${rowId}`
	...
	const handleDeleteClick = async (rowData) => {
		const url = `http://localhost:4004/cluster/deleteCluster?id=${rowData.id}`
	...
	const handleSaveCluster = async () => {
		if (!id) {
			const url = "http://localhost:4004/cluster/createCluster"
			const cluster = {
				title: title,
				image: image,
			}
			...
			const url = `http://localhost:4004/cluster/updateCluster?id=${id}`
			const cluster = {
				title: title,
				image: image,
			}
